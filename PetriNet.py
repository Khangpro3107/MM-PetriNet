    ...
    def draw(self, posX: int, posY: int, color=color_black, label_position='D') -> None:
        ...
        if label_position == 'D':
            print_text_box(self.name, posX + 20, posY +
                           transition_width + 18, color, font)
        else:
...
def item3(petri_net: PetriNet, input_free: int, input_busy: int, input_docu: int, input_wait: int, input_inside: int, input_done: int):
    petri_net.reset()
    firing_sequence = []
    start = Transition("start")
    change = Transition("change")
...
def item4(petri_net: PetriNet, input_free: int, input_busy: int, input_docu: int, input_wait: int, input_inside: int, input_done: int):
    petri_net.reset()
    print(
...
if __name__ == "__main__":
    ...
    while showing_menu:
        ...
        while flag_continue:
            ...
            if item_index == 0:
                if prompt_to_menu:
                    user_input, flag_continue, showing_menu = input_prompt1()
                else:
                    flag_continue = True
                ...
                if prompt_to_menu:
                    user_input, flag_continue, showing_menu = input_prompt2()
                else:
                    flag_continue = True
                if flag_continue and not showing_menu:
                ...
                if prompt_to_menu:
                    user_input, flag_continue, showing_menu = input_prompt3()
                else:
                    flag_continue = True
                if flag_continue and not showing_menu:
